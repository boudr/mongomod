...
builder::basic::document query_builder(ILuaBase* LUA, int iStackPos, bool update=false){
    int i = 1;
    ...
    for(LUA->PushNil(); LUA->Next(iStackPos) != 0; LUA->Pop()){
        if(LUA->IsType(-2, Type::STRING)){
            std::string field(LUA->GetString(-2));
            if(LUA->IsType(-1, Type::BOOL)){
                bool lbool= LUA->GetBool(-1);
                doc.append(kvp(field, types::b_bool{lbool}));
                i++;
                continue;
            }else if(LUA->IsType(-1, Type::NUMBER)){
                double lnum = LUA->GetNumber(-1);
                doc.append(kvp(field, types::b_double{lnum}));
                i++;
                continue;
            }else if(LUA->IsType(-1, Type::STRING)){
                doc.append(kvp(field, LUA->GetString(-1)));
                i++;
                continue;
            }else if(LUA->IsType(-1, Type::TABLE)){
                printf("\n%s\n", LUA->GetString(-2));

                doc.append(kvp(field, query_builder(LUA, -2)));
                i++;
                continue;
            }else{
            ...
            if(LUA->IsType(-1, Type::BOOL)){
                ...
                doc.append(kvp(field, LUA->GetString(-2)));
                i++;
                continue;
            }else if(LUA->IsType(-1, Type::TABLE)){
                /* if(update){ */
                ...
                /* }else{ */
                /* /1*     qBuilder.appendArray(kvp(LUA->GetString(-2), query_builder(LUA, -2))); *1/ */
                /* } */
                i++;
                continue;
            }else{
...
LUA_FUNCTION( Query ){
    ...
    mongocxx::cursor cursor = c->Query(collection, query_builder(LUA, 3));

    std::vector<document::view> elements;
